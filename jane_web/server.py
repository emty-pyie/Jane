...
class JaneRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        route_clean = route.rstrip("/") or "/"

        if route_clean in {"/", "/index", "/index.html"}:
            self._send_file(BASE_DIR / "templates" / "index.html", "text/html; charset=utf-8")
            return
        if route_clean == "/api/state":
            self._send_json(self.state.snapshot())
            return
        if route_clean == "/api/health":
            self._send_json({"status": "ok"})
            return
        if route_clean == "/static/style.css":
            self._send_file(BASE_DIR / "static" / "style.css", "text/css; charset=utf-8")
            return
        if route_clean == "/static/app.js":
        ...
        if not route_clean.startswith("/api/") and not route_clean.startswith("/static/"):
            self._send_file(BASE_DIR / "templates" / "index.html", "text/html; charset=utf-8")
            return
